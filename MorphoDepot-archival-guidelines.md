...
You choose the repository type when you create it. The choice is permanent.

A note on wording. The unit MorphoDepot manages is a **repository**: it is what you create, stage, publish, name, license, release, and cite, and it is the repository that is personal or archival. A **dataset** is the scientific content inside it — the scan and the segmentations built on it. A repository usually holds one dataset, but it does not have to.
...
| Storage for the scan | GitHub attachment, up to 2 GB | Jetstream2 cloud storage, currently capped at 10 GB, with off-site backup |
| Citable (DOI) | no | yes |
| Governance | yours alone, delete anytime | community-governed |
| Best for | teaching, drafts, experiments, anything you want to keep under your own control | work meant to last and be cited |

The two types never cross. There are no citable personal repositories. There are no disposable organization repositories. Membership unlocks the archival type. It does not force it. As a member you can still make personal repositories for casual or classroom work, or for anything else you would rather keep under your own control.

The rest of this guide is about archival repositories.
...
**Use Discussions for talk and Issues for work.** Open a Discussion in the organization for open-ended topics. Examples are a new terminology, a color convention, a workflow idea, or a call for collaborators. Open an Issue for concrete work on one repository. Examples are a segmentation task to assign, a problem to fix, or a release to plan. Discussions are for talking. Issues are for doing.
...
An archival repository is a shared resource under joint stewardship. That brings trade-offs that a personal repository does not have. Know them before you join.

**It is a one-way door.** While a repository is still private and staged, you can discard it freely. Once you publish it, you cannot quietly delete or hide it. Removing a published repository is an organization action. The curator cannot do it alone. You are making a durable commitment. That commitment is what makes a DOI meaningful.

**Publishing passes a short editorial check.** Think of a journal's handling editor. That editor confirms a submission is in good order before it is published. The editor is not a peer reviewer judging the science. The editor confirms the repository is well-formed. It has a real specimen and a real name. It uses recognized terminology. It has a license. The scan opens and is what it claims to be. The editor does not rule on whether a segmentation is anatomically perfect. This follows the spirit of the PLOS ONE model. The check is for validity and soundness. It does not judge the importance of the result.

**Accuracy is the community's job, and it improves over time.** Publishing certifies that a repository is sound and usable. It does not certify that its dataset is finished. If a segmentation can be improved, contributors correct and extend it. That is the point.
...
These are requirements. The app checks them automatically when you publish a repository or make a release. It re-runs the checks on your latest commit. A hard failure sends the request straight back to you with a list of fixes. No reviewer is involved. A soft finding does not block. It is flagged for the editor to look at. Design your repository to meet these from the start. That avoids round-trips.

### Name it for the specimen

Use a descriptive name based on the specimen. The pattern is Genus-species plus what the repository contains. A good example is Ariopsis-felis-cranium. Do not use names like my-repo, project1, or test123. The name is permanent and public. It is how others find and cite the repository. An automatic naming check is not in place yet. Until it is, the editor checks the name during review.
...
At creation you fill a short accession form. It records the species, the specimen type, the sex, the developmental stage, the acquisition modality, and the anatomical regions. If the specimen is accessioned, it also records a link to its record in a public collection database — GBIF, iDigBio, and Arctos are all understood, and the Darwin Core identifier is extracted from the record for the README. At publish, the species is checked against GBIF as a soft check, and any specimen-record URL is resolved as a soft check. Real, resolvable specimen metadata is what makes the repository useful and citable.

### Set a correct voxel size in millimeters

The scan must record an accurate voxel size in millimeters. The voxel size sets the real-world scale of the specimen. This is checked when you publish, and a scale that is not physically plausible is flagged for the editor. For example, a bee scanned at 1.0 mm per voxel over 2000 slices would describe a specimen about two meters long. That scale is wrong, and it will not pass review. Many scans are acquired in microns, so convert before you set it. A 10 micron voxel is 0.010 mm. Set the voxel size correctly in 3D Slicer before you create the repository — it is read from the scan at creation and is not re-read afterwards, so it cannot be corrected by editing later.

### Build any baseline on the source volume

A repository may publish as a bare scan with no segmentation. This lets the community start on it. If you do include a baseline segmentation, it must be built on the repository's own source volume. Its reference geometry must match the scan's dimensions. This is a hard check. The baseline's segments must also follow the terminology rules above.
...
**You get automatic credit.** No need to track who did what. When a curator accepts and merges an issue, the contributor is credited automatically at the next (and subsequent) release. Contributions by non-org members are credited by GitHub handle only, although the repository's curator can edit their information by hand. Org members are credited by their verified ORCID name and affiliation. The curator who created the repository, and who is responsible for reviewing its segmentations, is the lead author by default.

**Your releases get citable DOIs.** A repository becomes citable at its first release that contains a segmentation. Each release gets its own version DOI. There is also one concept DOI that always resolves to the latest version. Both are minted through Zenodo. Both appear on the repository's front-page README.md automatically, with a ready-to-paste citation.

**Larger volume sizes are supported.** The source scan lives in dedicated cloud object storage, so it is not subject to GitHub's 2 GB release-asset limit. The volume file is currently capped at 10 GB, for practical segmentation reasons — you typically need six to ten times the volume size in memory to work with it. That cap may be raised in future.
